...
namespace Combinatorics
{
	...
	public:
		...
		[[nodiscard]] std::enable_if_t<_Const, reference> operator*() const
		{
			return *(*_wrapped);
		}

		template<bool _Const = Const>
		[[nodiscard]] std::enable_if_t<!_Const, reference> operator*()
		{
			return *(*_wrapped);
		}

		template<bool _Const = Const>
		[[nodiscard]] std::enable_if_t<_Const, pointer> operator->() const
		{
			return *_wrapped;
		}

		template<bool _Const = Const>
		[[nodiscard]] std::enable_if_t<!_Const, pointer> operator->()
		{
			return *_wrapped;
		}
