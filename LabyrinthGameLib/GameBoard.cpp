...
namespace LabyrinthGame {
	...
	GameBoard::GameBoard()
		:
		_maze_pieces(initiallyCreateMazePieces()),
		_registered_token(),
		_maze(_maze_pieces),
	...
	void addEdgeIfTrueRemoveOtherwise(
		Combinatorics::Graph& g,
		Combinatorics::Vertex& v,
		Combinatorics::Vertex& w,
		bool condition)
	{
		if (condition)
		{
	...
	bool GameBoard::mazeConfigurationImpliesEdge(Coordinate const& c, Coordinate neighbor, Direction d) const
	{
		...
		return p.isOpen(d) &&
			(
				Utils::isValid(neighbor) ?
				getMazePiece(neighbor).isOpen(oppositeDirection(d)) :
				true
				);
	}
	...
	void GameBoard::insertSparePieceInRow(GameSettings::CoordType y, Direction direction, GameSettings::CoordType _spare_piece_id)
	{
	...
	void GameBoard::insertSparePieceInColumn(GameSettings::CoordType x, Direction direction, GameSettings::CoordType _spare_piece_id)
	{
